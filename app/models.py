...
class Profile(db.Model):
    ...
    full_name: so.Mapped[Optional[str]] = so.mapped_column(sa.String(120))
    bio: so.Mapped[Optional[str]] = so.mapped_column(sa.Text)
    avatar_url: so.Mapped[Optional[str]] = so.mapped_column(sa.String(256))
    birth_date: so.Mapped[Optional[datetime]] = so.mapped_column(sa.DateTime)
